...
@IsNodeHandler
@Component
public class TranslatePoints extends Fusion360Handler {
    ...
    public void handleNode(Node node) {
       //get in props count, xrangeLow, high,yrange low,high ,zrange low, high where low and high values are in same string separated by : in xrange...
        ...
        float dx = node.getData().getFloatProperty("dx");
        float dy = node.getData().getFloatProperty("dy");
        float dz = node.getData().getFloatProperty("dz");
        ...
        double[][] valuesTranslated = GeometryUtils.translate(values, dx, dy, dz);
