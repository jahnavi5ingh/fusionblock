...
@IsNodeHandler
@Component
public class PointOnCylinder extends Fusion360Handler {
    ...
    public void handleNode(Node node) {
       //get in props count, xrangeLow, high,yrange low,high ,zrange low, high where low and high values are in same string separated by : in xrange...
        ...
        float zheight = node.getData().getFloatProperty("zheight");
        int zcount = node.getData().getIntProperty("zcount");
        List<Point> values = new ArrayList<>();
        double [] [] pts = GeometryUtils.generateEquallySpacedPointsOnCircle(x,y,radius,count);
        for(int j = 0;j<zcount;j++){
            double zvalue = GeometryUtils.lerp(z,zheight,j);
            for (int i = 0; i < pts.length; i++) {
                values.add(new Point((float) pts[i][0], (float) pts[i][1], (float)zvalue));
            }
