...
@IsNodeHandler
@Component
public class RotatePoints extends Fusion360Handler {
    ...
    public void handleNode(Node node) {
       //get in props count, xrangeLow, high,yrange low,high ,zrange low, high where low and high values are in same string separated by : in xrange...
        List<Point> points = (List<Point>) super.getPointsFromInput(node.getData().getProperty("points"));
        float dx = node.getData().getFloatProperty("rx");
        float dy = node.getData().getFloatProperty("ry");
        float dz = node.getData().getFloatProperty("rz");
        ...
        double[][] valuesTranslated = GeometryUtils.rotate(values, dx, dy, dz);
